...
Page({
  ...
  loaddata: function(a) {
    ...
    utilRequest.NetRequest({
      url:""
      'pg.php/ZXinfo/session_page', 'post', data, '', function(res) {
      if (res.data.k1) {
  ...
  bottom: function() {
    var query = wx.createSelectorQuery() query.select('#hei').boundingClientRect() query.selectViewport().scrollOffset() query.exec(function(res) {
      wx.pageScrollTo({
  ...
  upimg1: function() {
    ...
    wx.chooseImage({
      success: function(res) {
        ...
        var tempFilePaths = res.tempFilePaths wx.uploadFile({
